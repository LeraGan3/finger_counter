...
while cap.isOpened():  # пока камера "работает"
    ...
    if multiLandMarks: # если руки найдены
        ...
        cv2.putText(image, lbl, (20, 140), cv2.FONT_HERSHEY_COMPLEX, 1, (70, 160, 46), 2)  #доработать с 2 руками
        cv2.putText(image, str(upCount), (30, 100), cv2.FONT_HERSHEY_COMPLEX, 4, (10, 60, 246), 2)
        print(upCount)

    currentTime = time.time()
    fps = 1 / (currentTime - prevTime)
    cv2.putText(image, f'FPS: {fps}', (10, 475), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 0), 1)
